...
SQLiteFillableStatement::SQLiteFillableStatement(sqlite3* _db, string const & sql) : nextBindIndex(1), nextColumnIndex(0), statement(NULL), db(_db) {
...
bool SQLiteFillableStatement::bindNext(long const & value) {
  int res = sqlite3_bind_int64(statement, nextBindIndex, value);
  nextBindIndex ++;
  return res ==  SQLITE_OK;
}

bool SQLiteFillableStatement::bindNext(std::string const & value) {
  int res = sqlite3_bind_text(statement, nextBindIndex, value.c_str(), -1, SQLITE_TRANSIENT);
  nextBindIndex ++;
  return res ==  SQLITE_OK;
}

bool SQLiteFillableStatement::bindNext(int const & value) {
  int res = sqlite3_bind_int(statement, nextBindIndex, value);
  nextBindIndex ++;
  return res ==  SQLITE_OK;
}

bool SQLiteFillableStatement::bindNext(double const & value) {
